...
function getItems(req, res, next) {
    var userId = req.userId;
    db.query("SELECT id, name, startDate, endDate, description, image__c AS image, campaignPage__c AS campaignPage, publishDate__c AS publishDate FROM wallet, salesforce1.campaign WHERE offerId = id AND userId=$1 AND type='Offer' AND status='In Progress' ORDER BY publishDate DESC LIMIT $2",
            [userId, 20])
